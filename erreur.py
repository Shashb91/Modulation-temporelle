...
def erreur1D(t, M_li = [100, 200, 400, 800, 1600], f = "LW"):
    ...
    for i in trange(nM, ncols = ncols):
        ...
        if f == "ADER4": eps[i, :] = np.sum([np.abs(data1.U[ti,n,:] - data2.U[ti,n,:]) for n in range(2,data1.M-2)])/(data1.M-4)
        else: eps[i, :] = np.sum([np.abs(data1.U[ti,n,:] - data2.U[ti,n,:]) for n in range(data1.M)])/data1.M
    return eps
