...
from modulation import rho_echelon, kappa_echelon, rho_echelon_moy, kappa_echelon_moy
...
def A2D_ms(data):
    def f(t):
        rho = (data.rho[0]*rho_echelon(data, data.eps_r[0])(t),data.rho[1]*rho_echelon(data, data.eps_r[1])(t), rho_echelon_moy(data)(t))
        kappa = (data.kappa[0]*kappa_echelon(data, data.eps_kappa[0])(t), data.kappa[1]*kappa_echelon(data, data.eps_kappa[1])(t), kappa_echelon_moy(data)(t))
        retour = []
        for i in [0,1,2]:
        ...
        for i in range(1, data.Mx - 1):
            for j in range(1, data.My - 1):
                ...
                if 1 <= (j + L//2) % (l + L) < L - 1:
                    ...
                    b2 = (0.5 * data.dt ** 2 * B[1] @ B[1]) @ ((data.U[n, i, j + 1, :] + data.U[n, i, j - 1, :] - 2 * data.U[n, i, j, :]) / data.dy ** 2)
                    b3 = 1 / (4 * data.dx * data.dy) * (0.5 * data.dt ** 2 * (A[1] @ B[1] + B[1] @ A[1])) @ (data.U[n, i + 1, j + 1] - data.U[n, i + 1, j - 1] - data.U[n, i - 1, j + 1] + data.U[n, i - 1, j - 1])
                else:
                    a1 = (data.dt / (2 * data.dx)) * A[2] @ (data.U[n, i + 1, j, :] - data.U[n, i - 1, j, :])
                    a2 = (data.dt / (2 * data.dy)) * B[2] @ (data.U[n, i, j + 1, :] - data.U[n, i, j - 1, :])
                    b1 = (0.5 * data.dt ** 2 * A[2] @ A[2]) @ ((data.U[n, i + 1, j, :] + data.U[n, i - 1, j, :] - 2 * data.U[n, i, j, :]) / data.dx ** 2)
                    b2 = (0.5 * data.dt ** 2 * B[2] @ B[2]) @ ((data.U[n, i, j + 1, :] + data.U[n, i, j - 1, :] - 2 * data.U[n, i, j, :]) / data.dy ** 2)
                    b3 = 1 / (4 * data.dx * data.dy) * (0.5 * data.dt ** 2 * (A[2] @ B[2] + B[2] @ A[2])) @ (data.U[n, i + 1, j + 1] - data.U[n, i + 1, j - 1] - data.U[n, i - 1, j + 1] + data.U[n, i - 1, j - 1])
        ...
        for i in range(2, data.Mx - 2):
            for j in range(2, data.My - 2):
                ...
                if 1 <= (j + L//2) % (l + L) < L - 1:
                    ...
                    a1 = - data.dt* (A[2] @ dxU[1] + B[2] @ dyU[1])
                    a2 = data.dt**2/2 * (A[2] @ A[2] @ dxU[2] + (A[2] @ B[2] + B[2] @ A[2]) @ dxyU[0] + B[2] @ B[2] @ dyU[2])
                    a3 = - data.dt**3/6 * (A[2] @ A[2] @ A[2] @ dxU[3] + B[2] @ B[2] @ B[2] @ dyU[3] + (A[2] @ A[2] @ B[2] + B[2] @ A[2] @ A[2]) @ dxyU[2] + (B[2] @ B[2] @ A[2] + A[2] @ B[2] @ B[2]) @ dxyU[1])
                    a4 = data.dt**4/24 * (A[2] @ A[2] @ A[2] @ A[2] @ dxU[4] + B[2] @ B[2] @ B[2] @ B[2] @ dyU[4] + (A[2] @ A[2] @ A[2] @ B[2] + B[2] @ A[2] @ A[2] @ A[2]) @ dxyU[4] + (A[2] @ A[2] @ B[2] @ B[2] + B[2] @ B[2] @ A[2] @ A[2]) @ dxyU[3] + (B[2] @ B[2] @ B[2] @ A[2] + A[2] @ B[2] @ B[2] @ B[2]) @ dxyU[5])
        ...
        for i in range(2, data.Mx - 2):
            for j in range(2, data.My - 2):
                ...
                dxyU = [coeff[5] * (U_temp[i-2, j-2,:] - 8*U_temp[i-1,j-2,:] + 8*U_temp[i+1,j-2,:] - U_temp[i+2,j-2,:]                                     #dxyU
                        ...
                        coeff[7] * (-U_temp[i-2,j-2,:] + 8*U_temp[i-2,j-1,:] - 8*U_temp[i-2,j+1,:] + U_temp[i-2,j+2,:]                                             #dxxxyU
                                    + 2*U_temp[i-1,j-2,:] - 16*U_temp[i-1,j-1,:] + 16*U_temp[i-1,j+1,:] - 2*U_temp[i-1,j+2,:]
                                    - 2*U_temp[i+1,j-2,:] + 16*U_temp[i+1,j-1,:] - 16*U_temp[i+1,j+1,:] + 2*U_temp[i+1,j+2,:]
                                    + U_temp[i+2,j-2,:] - 8*U_temp[i+2,j-1,:] +8*U_temp[i+2,j+1,:] - U_temp[i+2,j+2,:]),
                        coeff[7] * (-U_temp[i-2,j-2,:] + 2*U_temp[i-2,j-1,:] - 2*U_temp[i-2,j+1,:] + U_temp[i-2,j+2,:]                                             #dyyyxU
                                    + 8*U_temp[i-1,j-2,:] - 16*U_temp[i-1,j-1,:] + 16*U_temp[i-1,j+1,:] - 8*U_temp[i-1,j+2,:]
                                    - 8*U_temp[i+1,j-2,:] + 16*U_temp[i+1,j-1,:] - 16*U_temp[i+1,j+1,:] + 8*U_temp[i+1,j+2,:]
                                    + U_temp[i+2,j-2,:] - 2*U_temp[i+2,j-1,:] +2*U_temp[i+2,j+1,:] - U_temp[i+2,j+2,:])
                        ]
    ...
    for n in trange(0, data.N - 1, ncols=ncols):
        ...
        for i in range(0, data.Mx):
            for j in range(0, data.My):
                ...
                elif l + L - 2 >= (j + L // 2) % (l + L) > L + 1: U_temp[i, j, :] = np.diag([np.exp(-S_[0] * data.dt / 2), np.exp(-S_[1] * data.dt / 2), np.exp(-S_[2] * data.dt / 2)]) @ data.U[n, i, j, :]
                else: U_temp[i, j, :] = np.diag([np.exp(-_S[0] * data.dt / 2), np.exp(-_S[1] * data.dt / 2), np.exp(-_S[2] * data.dt / 2)]) @ data.U[n, i, j, :]
        ...
        for i in range(1, data.Mx - 1):
            for j in range(1, data.My - 1):
                ...
                if 1 <= (j + L//2) % (l + L) < L - 1:
                    ...
                    b2 = (0.5 * data.dt ** 2 * B[0] @ B[0]) @ ((U_temp[i, j + 1, :] + U_temp[i, j - 1, :] - 2 * U_temp[i, j, :]) / data.dy ** 2)
                    b3 = 1 / (4 * data.dx * data.dy) * (0.5 * data.dt ** 2 * (A[0] @ B[0] + B[0] @ A[0])) @ (U_temp[i + 1, j + 1] - U_temp[i + 1, j - 1] - U_temp[i - 1, j + 1] + U_temp[i - 1, j - 1])
                elif l + L - 2 >= (j + L // 2) % (l + L) > L + 1:
                    a1 = (1 / (2 * data.dx)) * (data.dt* A_[0] + data.dt**2/2*A_[1]) @ (U_temp[i + 1, j, :] - U_temp[i - 1, j, :])
                    a2 = (1 / (2 * data.dy)) * (data.dt* B_[0] + data.dt**2/2*B_[1]) @ (U_temp[i, j + 1, :] - U_temp[i, j - 1, :])
                    b1 = (0.5 * data.dt ** 2 * A_[0] @ A_[0]) @ ((U_temp[i + 1, j, :] + U_temp[i - 1, j, :] - 2 * U_temp[i, j, :]) / data.dx ** 2)
                    b2 = (0.5 * data.dt ** 2 * B_[0] @ B_[0]) @ ((U_temp[i, j + 1, :] + U_temp[i, j - 1, :] - 2 * U_temp[i, j, :]) / data.dy ** 2)
                    b3 = 1 / (4 * data.dx * data.dy) * (0.5 * data.dt ** 2 * (A_[0] @ B_[0] + B_[0] @ A_[0])) @ (U_temp[i + 1, j + 1] - U_temp[i + 1, j - 1] - U_temp[i - 1, j + 1] + U_temp[i - 1, j - 1])
                else:
                    a1 = (1 / (2 * data.dx)) * (data.dt* _A[0] + data.dt**2/2*_A[1]) @ (U_temp[i + 1, j, :] - U_temp[i - 1, j, :])
                    a2 = (1 / (2 * data.dy)) * (data.dt* _B[0] + data.dt**2/2*_B[1]) @ (U_temp[i, j + 1, :] - U_temp[i, j - 1, :])
                    b1 = (0.5 * data.dt ** 2 * _A[0] @ _A[0]) @ ((U_temp[i + 1, j, :] + U_temp[i - 1, j, :] - 2 * U_temp[i, j, :]) / data.dx ** 2)
                    b2 = (0.5 * data.dt ** 2 * _B[0] @ _B[0]) @ ((U_temp[i, j + 1, :] + U_temp[i, j - 1, :] - 2 * U_temp[i, j, :]) / data.dy ** 2)
                    b3 = 1 / (4 * data.dx * data.dy) * (0.5 * data.dt ** 2 * (_A[0] @ _B[0] + _B[0] @ _A[0])) @ (U_temp[i + 1, j + 1] - U_temp[i + 1, j - 1] - U_temp[i - 1, j + 1] + U_temp[i - 1, j - 1])
                U_temp_[ i, j, :] = U_temp[i, j, :] - a1 - a2 + b1 + b2 + b3 + s
        ...
        for i in range(0, data.Mx):
            for j in range(0, data.My):
                if 1 <= (j + L//2) % (l + L) <= L: data.U[n + 1, i, j, :] = np.diag([np.exp(-S[0] * data.dt / 2), np.exp(-S[1] * data.dt / 2), np.exp(-S[2] * data.dt / 2)]) @ U_temp_[i, j, :]
                elif  l + L - 2 >= (j + L // 2) % (l + L) > L + 1: data.U[n + 1, i, j, :] = np.diag([np.exp(-S_[0] * data.dt / 2), np.exp(-S_[1] * data.dt / 2), np.exp(-S_[2] * data.dt / 2)]) @ U_temp_[i, j, :]
                else: data.U[n + 1, i, j, :] = np.diag([np.exp(-_S[0] * data.dt / 2), np.exp(-_S[1] * data.dt / 2), np.exp(-_S[2] * data.dt / 2)]) @ U_temp_[i, j, :]

    rho1 = [data.rho[0]*rho_echelon(data, data.eps_r[0])(data.dt * n)[0] for n in range(data.N)]
    rho2 = [data.rho[1]*rho_echelon(data, data.eps_r[1])(data.dt * n)[0] for n in range(data.N)]
    rho3 = [rho_echelon_moy(data)(data.dt * n)[0] for n in range(data.N)]
    kappa1 = [data.kappa[0] * kappa_echelon(data, data.eps_kappa[0])(data.dt * n)[0] for n in range(data.N)]
    kappa2 = [data.kappa[1] * kappa_echelon(data, data.eps_kappa[1])(data.dt * n)[0] for n in range(data.N)]
    kappa3 = [kappa_echelon_moy(data)(data.dt * n)[0] for n in range(data.N)]
    data.E = np.zeros(data.N)
    for j in range(data.My):
        for n in range(data.N):
            if 1 <= (j + L//2) % (l + L) <= L: data.E[n] += np.sum(0.5 * rho1[n] * (data.U[n, :, j, 0] ** 2 + data.U[n, :, j, 1] ** 2) + 0.5 * data.U[n, :, j, 2] ** 2/kappa1[n], axis = 0)
            elif l + L - 2 >= (j + L // 2) % (l + L) > L + 1: data.E[n] += np.sum(0.5 * rho2[n] * (data.U[n, :, j, 0] ** 2 + data.U[n, :, j, 1] ** 2) + 0.5 * data.U[n, :, j, 2] ** 2/kappa2[n], axis = 0)
            else: data.E[n] += np.sum(0.5 * rho3[n] * (data.U[n, :, j, 0] ** 2 + data.U[n, :, j, 1] ** 2) + 0.5 * data.U[n, :, j, 2] ** 2/kappa3[n], axis = 0)
        ...
        for i in range(2, data.Mx - 2):
            for j in range(2, data.My - 2):
                ...
                if 1 <= (j + L//2) % (l + L) < L - 1:
                    ...
                    a1 = - data.dt * (A_[0] @ dxU[1] + B_[0] @ dyU[1])
                    a2 = data.dt**2/2 * (- A_[1] @ dxU[1] - B_[1] @ dyU[1] + A_[0] @ A_[0] @ dxU[2] + B_[0] @ B_[0] @ dyU[2] + (A_[0] @ B_[0] + B_[0] @ A_[0]) @ dxyU[0])
                    a3 = data.dt**3/6 * (- A_[2] @ dxU[1] - B_[2] @ dyU[1] + (2 * A_[1] @ A_[0] + A_[0] @ A_[1]) @ dxU[2] + (2 * B_[1] @ B_[0] + B_[0] @ B_[1]) @ dyU[2]
                                     + (2 * A_[1] @ B_[0] + A_[0] @ B_[1] + 2 * B_[1] @ A_[0] + B_[0] @ A_[1]) @ dxyU[0]
                                     - (A_[0] @ A_[0] @ B_[0] + B_[0] @ A_[0] @ A_[0]) @ dxyU[2] - (B_[0] @ B_[0] @ A_[0] + A_[0] @ B_[0] @ B_[0]) @ dxyU[1]
                                     - A_[0] @ A_[0] @ A_[0] @ dxU[3] - B_[0] @ B_[0] @ B_[0] @ dyU[3])
                    a4 = data.dt**4/24 * (- A_[3] @ dxU[1] - B_[3] @ dyU[1]
                                      + (3 * A_[2] @ B_[0] + 3 * A_[1] @ B_[1] + A_[0] @ B_[2] + 3 * B_[2] @ A_[0] + 3 * B_[1] @ A_[1] + B_[0] @ A_[2]) @ dxyU[0]
                                      + (3 * A_[2] @ A_[0] + 3 * A_[1] @ A_[1] + A_[0] @ A_[2]) @ dxU[2] + (3 * B_[2] @ B_[0] + 3 * B_[1] @ B_[1] + B_[0] @ B_[2]) @ dyU[2]
                                      - (3 * A_[1] @ A_[0] @ A_[0] + 2 * A_[0] @ A_[1] @ A_[0] + A_[0] @ A_[0] @ A_[1]) @ dxU[3]
                                      - (3 * B_[1] @ B_[0] @ B_[0] + 2 * B_[0] @ B_[1] @ B_[0] + B_[0] @ B_[0] @ B_[1]) @ dyU[3]
                                      - (3 * A_[1] @ A_[0] @ B_[0] + 2 * A_[0] @ A_[1] @ B_[0] + A_[0] @ A_[0] @ B_[1] + 3 * B_[0] @ B_[0] @ A_[1] + 2 * B_[0] @ A_[1] @ A_[0] + B_[0] @ A_[0] @ A_[1]) @ dxyU[2]
                                      - (3 * B_[1] @ B_[0] @ A_[0] + 2 * B_[0] @ B_[1] @ A_[0] + B_[0] @ B_[0] @ A_[1] + 3 * A_[0] @ A_[0] @ B_[1] + 2 * A_[0] @ B_[1] @ B_[0] + A_[0] @ B_[0] @ B_[1]) @ dxyU[1]
                                      + (A_[0] @ A_[0] @ A_[0] @ B_[0] + B_[0] @ A_[0] @ A_[0] @ A_[0]) @ dxyU[4] + (B_[0] @ B_[0] @ B_[0] @ A_[0] + A_[0] @ B_[0] @ B_[0] @ B_[0]) @ dxyU[5]
                                      + A_[0] @ A_[0] @ A_[0] @ A_[0] @ dxU[4] + B_[0] @ B_[0] @ B_[0] @ B_[0] @ dyU[4])
                else:
                    a1 = - data.dt * (_A[0] @ dxU[1] + _B[0] @ dyU[1])
                    a2 = data.dt**2/2 * (- _A[1] @ dxU[1] - _B[1] @ dyU[1] + _A[0] @ _A[0] @ dxU[2] + _B[0] @ _B[0] @ dyU[2] + (_A[0] @ _B[0] + _B[0] @ _A[0]) @ dxyU[0])
                    a3 = data.dt**3/6 * (- _A[2] @ dxU[1] - _B[2] @ dyU[1] + (2 * _A[1] @ _A[0] + _A[0] @ _A[1]) @ dxU[2] + (2 * _B[1] @ _B[0] + _B[0] @ _B[1]) @ dyU[2]
                                     + (2 * _A[1] @ _B[0] + _A[0] @ _B[1] + 2 * _B[1] @ _A[0] + _B[0] @ _A[1]) @ dxyU[0]
                                     - (_A[0] @ _A[0] @ _B[0] + _B[0] @ _A[0] @ _A[0]) @ dxyU[2] - (_B[0] @ _B[0] @ _A[0] + _A[0] @ _B[0] @ _B[0]) @ dxyU[1]
                                     - _A[0] @ _A[0] @ _A[0] @ dxU[3] - _B[0] @ _B[0] @ _B[0] @ dyU[3])
                    a4 = data.dt**4/24 * (- _A[3] @ dxU[1] - _B[3] @ dyU[1]
                                      + (3 * _A[2] @ _B[0] + 3 * _A[1] @ _B[1] + _A[0] @ _B[2] + 3 * _B[2] @ _A[0] + 3 * _B[1] @ _A[1] + _B[0] @ _A[2]) @ dxyU[0]
                                      + (3 * _A[2] @ _A[0] + 3 * _A[1] @ _A[1] + _A[0] @ _A[2]) @ dxU[2] + (3 * _B[2] @ _B[0] + 3 * _B[1] @ _B[1] + _B[0] @ _B[2]) @ dyU[2]
                                      - (3 * _A[1] @ _A[0] @ _A[0] + 2 * _A[0] @ _A[1] @ _A[0] + _A[0] @ _A[0] @ _A[1]) @ dxU[3]
                                      - (3 * _B[1] @ _B[0] @ _B[0] + 2 * _B[0] @ _B[1] @ _B[0] + _B[0] @ _B[0] @ _B[1]) @ dyU[3]
                                      - (3 * _A[1] @ _A[0] @ _B[0] + 2 * _A[0] @ _A[1] @ _B[0] + _A[0] @ _A[0] @ _B[1] + 3 * _B[0] @ _B[0] @ _A[1] + 2 * _B[0] @ _A[1] @ _A[0] + _B[0] @ _A[0] @ _A[1]) @ dxyU[2]
                                      - (3 * _B[1] @ _B[0] @ _A[0] + 2 * _B[0] @ _B[1] @ _A[0] + _B[0] @ _B[0] @ _A[1] + 3 * _A[0] @ _A[0] @ _B[1] + 2 * _A[0] @ _B[1] @ _B[0] + _A[0] @ _B[0] @ _B[1]) @ dxyU[1]
                                      + (_A[0] @ _A[0] @ _A[0] @ _B[0] + _B[0] @ _A[0] @ _A[0] @ _A[0]) @ dxyU[4] + (_B[0] @ _B[0] @ _B[0] @ _A[0] + _A[0] @ _B[0] @ _B[0] @ _B[0]) @ dxyU[5]
                                      + _A[0] @ _A[0] @ _A[0] @ _A[0] @ dxU[4] + _B[0] @ _B[0] @ _B[0] @ _B[0] @ dyU[4])
